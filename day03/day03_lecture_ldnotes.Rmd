...
```{r} 
# little tick shere tell the mark down that you are doing a chunk of code
# can use echo = FALSE if you don't want to show code in the document 
...
ttest.results # can comment all the hard work out and re-load the prior workspace image
rm(ttest.results)
ttest.results
...
# can use round command to get around floating point error 
...
sample(10) # can specify whether you want replacement or not 
rnorm(10)
rbinom(5, size = 10, prob = .2) #heads or tail, with 5 numbers from 10 flips and probability of success
...
quantile(x, seq(0, 1, .1)) # if you want to look at quantiles at each 10%, for instance 
